...
let fhirResources = JSON.parse(fs.readFileSync('definitions-R4/profiles-resources.json'));
var resourceEntries = fhirResources.entry.filter(entry => entry.resource.kind === "resource" && entry.resource.abstract === false);
let fhirTypes = JSON.parse(fs.readFileSync('definitions-R4/profiles-types.json'));
...
function fhirProperties(elementPath, typeCode, card, flag) {
    ...
    output += `<version>R4</version><scope>core</scope><textkey>${elementPath}</textkey></fhirProperties>`;
    console.log(output);
    _order++;
}