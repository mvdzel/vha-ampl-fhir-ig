...
valuesetNames.forEach(name => {
    var valueset = valuesets[name];
    fs.writeFileSync("output/ValueSet-" + name + ".json", JSON.stringify(valueset, null, 2));
});
    ...
    if (element == undefined) {
        ...
        if (proplines.length > 0 && elementPath.indexOf(".extension.") != -1) {
            ...
            element = elementsByPath[profileId][elementPath] = {
                id: `${prefix}.extension:${extname}`, // id will be automatically generated by the IG Publisher!
                path: `${prefix}.extension`,
                sliceName: extname.replace(/\./g,'-'), // Forge doesnot like '/' here
                type: [
                    {
                        code: "Extension",
                        profile: exturi
...
profileIds.forEach(profileId => {
    ...
    fs.writeFileSync("output/StructureDefinition-" + profileId + ".json", JSON.stringify(entry, null, 2));
});
...
conceptmapNames.forEach(name => {
    var conceptmap = conceptmaps[name];
    fs.writeFileSync("output/ConceptMap-" + name + ".json", JSON.stringify(conceptmap, null, 2));
});
